...
class SLAMSampleBuilder:
    """
    A customized sample builder for SLAM, which includes both 3D obb detection task and surface reconstruction task along with segmentations.
    ...
    def __init__(
        ...
        depth_vrs_file: str,
        segmentation_vrs_file: str,
        synthetic_vrs_file: str,
    ) -> None:
        """
        Initializes the SLAMSampleBuilder: with necessary configuration and file paths.
        ...
        Args:
            ...
            segmentation_vrs_file [Optional]: Path to the depth VRS file. This is required for the surface recon task.
            synthetic_vrs_file [Optional]: Path to the depth VRS file. This is required for the surface recon task.
        """
        self.conf = conf

        self.vrs_file = vrs_file
        self.depth_vrs_file = depth_vrs_file
        self.segmentation_vrs_file = segmentation_vrs_file
        self.synthetic_vrs_file = synthetic_vrs_file
    ...
    def _add_processors_from_conf(
        ...
        if (
            ...
            # Synthetic processor
        if (
            "rgb_synthetic" in conf
            and conf.rgb_synthetic.selected
            and os.path.exists(self.synthetic_vrs_file)
        ):
            assert (
                self.synthetic_vrs_file != ""
            ...
            selected_camera_label_to_stream_ids["rgb_synthetic"] = (
                    processors["rgb_synthetic"].get_stream_id()
                )
